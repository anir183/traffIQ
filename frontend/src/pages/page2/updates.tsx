...
const RECENT_API_URL =
  ...
  "http://localhost:8080/api/detections/recent";
  ...
  return {
    ...
    plate: {
      ...
      confidence: Math.round(
        Number(raw.plateConfidence ?? 0) * 100
      ),
...
function AnprLog() {
  const [source, setSource] =
    useState<FeedSource>("connecting");
  ...
  useEffect(() => {
     ...
     * =========================================
     *
     * Backend returns the latest 15
     * detections in DESCENDING order:
     *
     * 10:45
     * 10:40
     * 10:35
    ...
    const loadRecentDetections =
      async () => {
        try {
          ...
          const data =
            (await response.json()) as RecentDetection[];
          ...
          client.subscribe(
            ...
            (message) => {
              try {
                ...
                console.error(
                  "Failed to parse detection message:",
                  err
                );
              }
            }
