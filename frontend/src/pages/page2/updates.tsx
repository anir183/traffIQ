...
// const WS_URL = "ws://localhost:8080/ws"; //https://websockettest-kkcj.onrender.com/api/detections
// const TOPIC = "/topic/detections";

const WS_URL = "wss://websockettest-kkcj.onrender.com/ws"; 
const TOPIC = "/topic/detections";
  ...
  useEffect(() => {
    const client = new Client({
      ...
      onConnect: () => {
        ...
        client.subscribe(TOPIC, (message) => {
          try {
            ...
            // FIX: If data is an array, spread it (...data) so it flattens into the prev list.
            // If it's a single object, just put it in an array [data].
            setItems((prev) => {
      ...
      onStompError: (frame) => {
        ...
        setError("Failed to connect to live detections feed");
      },
            ...
            <div className="flex items-center gap-1">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <button
                  ...
                  type="button"
                  onClick={() => setPage(p)}
                  className={`h-6 w-6 rounded-full text-xs transition-colors ${p === safePage
                      ? "bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900"
                      : "text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800"
                    }`}
                >
