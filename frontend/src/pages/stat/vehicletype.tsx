...
export default function VehicleTypeBreakdown() {
  const segments = useMemo(() => {
    const cumulativeBefore = SEGMENTS.map((_, i) =>
      SEGMENTS.slice(0, i).reduce((acc, s) => acc + s.percent, 0)
    )
      ...
      <div className="flex min-w-0 flex-wrap items-center gap-4">
        <div className="relative shrink-0" style={{ width: 140, height: 140 }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-xl font-semibold text-slate-900">{TOTAL_VEHICLES}</span>
        ...
        <div className="flex min-w-0 flex-col gap-2.5">
          {segments.map((segment) => (
            <div key={segment.label} className="flex min-w-0 items-center justify-between gap-4 text-sm">
              ...
              <span className="font-medium text-slate-900">{segment.percent}%</span>
            </div>
          ))}
        </div>
      </div>
