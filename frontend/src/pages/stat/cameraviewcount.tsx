  ...
  return (
    <div className="flex flex-col rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100">Camera-wise Vehicle Count</h3>
      </div>
      ...
      <div className="flex flex-col gap-4">
        {DATA.map((row) => {
          ...
          return (
            <div key={row.camera}>
              <span className="mb-1 block text-sm text-slate-500 dark:text-slate-400">{row.camera}</span>
