...
const HARDCODED_INCIDENTS: Incident[] = [
  ...
  { id: '6', icon: 'warning', title: 'Signal Malfunction', detail: 'Intersection A12', location: 'Howrah Bridge', time: '14:05', status: 'Active' },
  { id: '7', icon: 'alert', title: 'Blacklisted Vehicle', detail: 'HR26DD2233', location: 'Salt Lake', time: '14:02', status: 'Active' },
  { id: '8', icon: 'warning', title: 'Accident', detail: 'Two-wheeler down', location: 'AJC Bose Road', time: '13:40', status: 'Investigating' },
  { id: '9', icon: 'alert', title: 'Speed Violation', detail: 'WB01BB5566', location: 'Kona Expressway', time: '13:31', status: 'Investigating' },
  { id: '10', icon: 'warning', title: 'Road Construction', detail: 'Lane closure', location: 'Ballygunge', time: '12:15', status: 'Resolved' },
  { id: '11', icon: 'warning', title: 'Signal Malfunction', detail: 'Intersection B7', location: 'Park Circus', time: '11:58', status: 'Resolved' },
  { id: '12', icon: 'wrongway', title: 'Wrong Way', detail: 'Unknown Vehicle', location: 'Dhakuria', time: '13:58', status: 'Active' },
  { id: '13', icon: 'wrongway', title: 'Route Anomaly', detail: 'MH02CX8899', location: 'City Centre', time: '13:20', status: 'Investigating' },
  { id: '14', icon: 'warning', title: 'Accident', detail: 'Minor collision', location: 'Ruby More', time: '11:30', status: 'Resolved' },
  { id: '15', icon: 'alert', title: 'Speed Violation', detail: 'DL4MA9900', location: 'Ballygunge Phari', time: '13:50', status: 'Active' },
]
...
export default function RecentIncidents({ filter = 'all' }: { filter?: FilterKey }) {
  ...
  return (
    <div className="flex w-full max-w-sm min-h-0 flex-col rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      ...
      <div ref={containerRef} className="min-h-0 flex-1 divide-y divide-slate-100 overflow-y-auto dark:divide-slate-800">
        {pageItems.map((incident) => (
          <IncidentRow key={incident.id} incident={incident} />
        ))}
      </div>
      {incidents.length === 0 && (
        <p className="flex-1 py-8 text-center text-sm text-slate-400 dark:text-slate-500">No incidents match this filter.</p>
      )}
      ...
      {incidents.length > 0 && (
        <div className="mt-auto flex items-center justify-between gap-2 border-t border-slate-100 pt-3 text-xs dark:border-slate-800">
          <span className="text-slate-400 dark:text-slate-500">
            Showing {startIndex + 1}–{startIndex + pageItems.length} of {incidents.length}
          </span>
