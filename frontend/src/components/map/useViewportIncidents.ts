...
export function useViewportIncidents({
  ...
  const handlersRef = useRef<{ load: () => void; moveend: () => void } | null>(
    null,
  );
  ...
  const runFetch = useCallback(
    (reason: "move" | "poll" | "force") => {
      ...
      const bounds = clampBoundsArea(
        paddedBounds(bboxFromMap(map), paddingFraction),
      );
      ...
      const isFresh =
        ...
        now - lastSuccessAtRef.current < pollMs * 0.8;
      ...
      if (coolingDown) {
        ...
        const remaining = Math.max(
          cooldownMs - (now - lastFetchAtRef.current),
          100,
        );
